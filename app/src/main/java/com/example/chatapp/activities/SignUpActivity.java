...
public class SignUpActivity extends AppCompatActivity {
    ...
    private boolean isValidEmail(String email) {
        String emailPattern = "[a-zA-Z\\d._%+-]+@[a-zA-Z\\d.-]+\\.[a-zA-Z]{2,}";

        // Ajoutez une vérification supplémentaire pour les caractères spéciaux
        if (!email.matches(emailPattern) || email.contains("--") || email.contains(";") || email.contains("'")) {
            return false;
        }
    ...
    private boolean isEmailAlreadyExists(String email) {
        ...
        try {
            // Utilisez Tasks.await pour obtenir le résultat de manière synchrone
            QuerySnapshot querySnapshot = Tasks.await(query.get());

            // Vérifiez si le snapshot n'est pas vide
