...
public class SignInActivity extends AppCompatActivity {
    ...
    // Ajoutez ces variables au début de votre classe
    private int loginAttempts = 0;
    private static final int MAX_LOGIN_ATTEMPTS = 3; // Définissez le nombre maximal de tentatives autorisées
    private static final long LOCKOUT_DURATION = 10000; // Délai d'attente après un blocage (30 secondes ici)
    private long lastLoginAttemptTimestamp = 0;
    private boolean isSignInButtonBlocked = false;
    ...
    private void signInWithEmail() {
        ...
        String ipAddress = getClientIpAddress(); // Méthode à implémenter pour obtenir l'adresse IP du client

        DoSProtection doSProtection = new DoSProtection();

        if (!doSProtection.isAllowed(ipAddress)) {
            showToast("Too many requests from your IP address. Please wait before trying again.");
            loading(false);
            return;
        }

        if (System.currentTimeMillis() - lastLoginAttemptTimestamp < LOCKOUT_DURATION) {
            showToast("Too many failed login attempts. Please wait before trying again.");
            loading(false);
            return;
        }
                ...
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (loginAttempts >= MAX_LOGIN_ATTEMPTS) {
                            lastLoginAttemptTimestamp = System.currentTimeMillis();
                            showToast("Too many failed login attempts. Your account is temporarily blocked.");
                        } else {
    ...
    private void disableSignInButton() {
        ...
        binding.buttonSignIn.setBackgroundResource(android.R.color.darker_gray); // Remplacez la couleur grise par celle que vous souhaitez
    }

    // Assurez-vous de réactiver le bouton lorsque le compte est débloqué ou après un certain temps
    private void enableSignInButton() {
        binding.buttonSignIn.setEnabled(true);
        binding.buttonSignIn.setBackgroundResource(R.color.solidCircle); // Remplacez R.color.votre_couleur par votre couleur
    }
    ...
    private void checkIfUserExists(String email) {
        ...
        database.collection(Constants.KEY_COLLECTION_USERS)
                ...
                .limit(1) // Limitez le nombre de résultats à 1, car l'email devrait être unique
                .get()
